...
public class MouseLook : MonoBehaviour {
	...
	void Update() {
        if(Input.GetMouseButton(1)) {
			...
			//Debug.Log(mouse.position.x.ReadValue());
            if (axes == RotationAxes.MouseXAndY)
            {
                // Read the mouse input axis
                rotationX += Input.mousePosition.x * sensitivityX * Time.deltaTime;
                rotationY += Input.mousePosition.y * sensitivityY * Time.deltaTime;
